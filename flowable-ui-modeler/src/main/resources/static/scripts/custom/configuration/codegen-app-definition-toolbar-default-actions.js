...
var CODEGEN_APP_DEFINITION_TOOLBAR = {
    ACTIONS: {
        ...
        saveModel: function (services) {
            ...
            _internalCreateModal({
                backdrop: true,
                keyboard: true,
                template: 'views/custom/popup/codegen-app-definition-save-model.html?version=' + Date.now(),
        ...
        closeEditor:  function (services) {
        	services.$location.path('/codegen-apps');
        }
    }
};

/** Custom controller for the save dialog */
angular.module('flowableModeler').controller('SaveCodeGenAppDefinitionCtrl',
    ...
    if ($rootScope.currentAppDefinition.description) {
    	...
    	projectName = description.split('|')[0];
    	basePackage = description.split('|')[1];
    	outputDir = description.split('|')[2];
    ...
    $scope.save = function (saveCallback, force) {
        ...
        $rootScope.currentAppDefinition.description = $scope.saveDialog.projectName + '|' + $scope.saveDialog.basePackage + '|' + $scope.saveDialog.outputDir;
        var data = {
