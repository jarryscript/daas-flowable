...
angular.module('flowableModeler')
  .controller('CodeGenAppDefinitionCtrl', ['$rootScope', '$scope', '$translate', '$http', '$location', '$routeParams', '$modal', '$popover', '$timeout',
    ...
    $scope.loadApp = function() {
    	...
    	$http({method: 'GET', url: url}).
        	success(function(data, status, headers, config) {
        		...
        		$scope.model.app.projectName = data.description.split('|')[0];
        		$scope.model.app.basePackage = data.description.split('|')[1];
        		$scope.model.app.outputDir = data.description.split('|')[2];
        		$scope.loadVersions();
    ...
    $scope.openEditor = function() {
        $location.path("/codegen-app-editor/" + $scope.model.latestModelId);
    };
